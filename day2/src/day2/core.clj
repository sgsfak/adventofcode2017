...
(defn solution1 [input]
  (let [lines (line-seq (BufferedReader. (StringReader. input)))
        rows (for [line lines] (->> (clojure.string/split line #"\s") (map #(Integer/parseInt %))))
        maxes (map #(apply max %) rows)
        mins (map #(apply min %) rows)]
    (reduce + (map - maxes mins))))


(defn evenly-divide? [a b]
  (if (> b a) (evenly-divide? b a)
    (let [rem  (mod a b)]
     (if (= 0 rem) (/ a b) nil))))

(defn row-check [row]
  (let [n (count row)
        indices (for [i (range n) j (range (inc i) n)] [i j])]
    (loop [p indices]
      (if (empty? p) 0
        (let [[i j] (first p)
              rp (rest p)]
          (if-let [rem (evenly-divide? (row i) (row j))]
            rem
            (recur rp)))))))

(defn solution2 [input]
  (let [lines (line-seq (BufferedReader. (StringReader. input)))
        rows (for [line lines] (->> (clojure.string/split line #"\s") (mapv #(Integer/parseInt %))))
        m (map row-check rows)]
    (reduce + 0 m)))
